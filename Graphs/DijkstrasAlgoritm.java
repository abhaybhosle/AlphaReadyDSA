...
public class DijkstrasAlgoritm {
    ...
    static class Pair implements Comparable<Pair> {
        ...
        public int compareTo(Pair p2) {
            return this.path - p2.path; // path based sorting for my pairs. Shortest distance from node 0 to neighbours
    ...
    public static void dijkstras(ArrayList<Edge> graph[], int src) {
        int distance[] = new int[graph.length];// dist[i] -> src to i
        for (int i = 0; i < graph.length; i++) {
            if (i != src) {
            ...
            if (!vis[curr.n]) {
                ...
                for (int i = 0; i < graph[curr.n].size(); i++) {
                    ...
                    if (distance[u] + wt < distance[v]) { // update distance of src to v
                        distance[v] = distance[u] + wt; //// formula for dijkstras algorithm
